...
const create_files = async (req, res, next) => {
  try {
    ...
    await Promise.all(
      req.files.map((v) => Models.File.create_files(user_id, v.filename))
    );
...
// POST: "/:user_id/nickname" 유저의 프로필 이미지 등록 요청 시 응답 (이미지 등록)
// PATCH: "/:user_id/nickname" 유저의 프로필 이미지 변경 요청 시 응답 (이미지 등록)
